...
// ACTIONS - auto generated
    ...
    public void addMessage(String originator, String message) {
        checkVisible();
        String none = "\nNone";
        String gdb = "gdbMailbox:";
        if(message.startsWith(gdb)) {
            message = message.substring(gdb.length(), message.length());
        }
        if(message.endsWith(none)) {
            message = message.substring(0, (message.length() - none.length()));
        }
